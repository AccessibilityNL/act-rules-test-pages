...
(function ($) {
  ...
  var menu_selector = '.menu-name-main-menu';
  $(document).ready(function () {
    $( menu_selector + ' .depth-3:not(.active, .menu-dummy)').find('.menu').addClass('hidden');
    $(document).on('click', menu_selector + ' .nolink', function () {
      var dummy = $(this);
      if (dummy.hasClass('active-trail')) {
        if (dummy.hasClass('close-item')) {
          dummy.addClass('open-item');
          dummy.setAttribute('aria-expanded', 'true');
          dummy.removeClass('close-item');
          dummy.next('.menu').slideDown('300');
        }
        else {
          dummy.addClass('close-item');
          dummy.removeClass('open-item');
          dummy.setAttribute('aria-expanded', 'false');
          dummy.next('.menu').slideUp('300');
        }
      } else {

        if (dummy.hasClass('open-item')) {
          dummy.addClass('close-item');
          dummy.removeClass('open-item');
          dummy.setAttribute('aria-expanded', 'false');
          dummy.next('.menu').slideUp('300');
    ...
    /**
     * Click function to open and close faq answers related by taxonomy term.
     */

      // Hide default all answers.
    $('.view-faq-per-pagina').find('.faq-antwoord').hide();
    $('.view-faq-per-pagina .views-field-title').attr('aria-expanded', 'false');
    $('.view-faq-per-pagina .views-field-title').append('<p class="visibility-hidden">Klik voor het antwoord</p>');
    // Add click on question.
    $('.view-faq-per-pagina .views-field-title').bind("click", function () {
      if ($(this).hasClass('active')) {
        $(this).removeClass('active').siblings().slideUp(300);
        $(this).setAttribute('aria-expanded', 'false');
      } else {
        $(this).addClass('active').siblings().slideDown(300);
    ...
    */
    var hasLooped = false;
    $("#page-content .tabnav ul li a").keydown(function(e) {
      var keyCode = e.keyCode || e.which, $this = $(this);
      // Check if user is tabbing
      if (keyCode === 9) {
        /*
        * Set focus on items inside the tab-content
        * focusOn: HTML elements that needs focus in content
        */
        var index = $this.closest("li").index(),
            $tabContent = $(".tab-content .tab-pane").eq(index),
            $result = $tabContent.find(":focusable").filter(":visible");
        ...
        if ($result.length > 0) {
          ...
          // No focus found
          if (index === $this.closest("ul").find("li").length - 1) {
            // Last tab tabbed prepare hasLooped for re-tabbing
            hasLooped = true;
            return;
          } else {
            e.preventDefault();
    ...
    $("#page-content .tabnav ul li a").keyup(function(e) {
      var keyCode = e.keyCode || e.which, $this = $(this);
      // Check if user is tabbing
      if (keyCode === 9 && hasLooped) {
        // Reset tabbing
        $(this).click();
        hasLooped = false;
      }
    ...
    $("#page-content .tabnav ul li").keyup(function(e) {
      var keyCode = e.keyCode || e.which, $this = $(this);
      // Check if user is tabbing
      if (keyCode === 9) {
        console.log($("#page-content .tabnav ul li").eq($(this).index()).last());
        $("#page-content .tabnav ul li").eq($(this).index()).last();
      }
    });
    ...
    */
    ...
    /* ARIA */
    if (isMobile) {
      var $mainMenuItems = $("nav[role='navigation'] > ul.menu li.parent-dropdown");

      for (var i = 0; i < $mainMenuItems.length; i++) {
        var $iItem = $mainMenuItems.eq(i);

        $iItem.find("> ul").attr("aria-hidden", "true");
        $iItem.find("> a").attr("aria-expanded", "false");
      }
    }

    function setPageArias() {
      var $mainMenu = $("nav[role='navigation'] > ul.menu");

      $mainMenu.find("li.parent-dropdown > a").attr("aria-haspopup", "true");
      $mainMenu.find("li.parent-dropdown > ul").attr({"aria-hidden": "true", "aria-label": "submenu"});
    } setPageArias();

    $(document).on("mouseenter", "nav[role='navigation'] > ul.menu li.parent-dropdown", function() {
      if ($(window).width() >= 768) {
        $(this).find("> ul").attr("aria-hidden", "false");
      }
    });

    $(document).on("mouseleave", "nav[role='navigation'] > ul.menu li.parent-dropdown", function() {
      if ($(window).width() >= 768) {
        $(this).find("> ul").attr("aria-hidden", "true");
      }
    });

    $(document).on("click touchstart", "nav[role='navigation'] > ul.menu li.parent-dropdown", function() {
      // Device fix
      if (isMobile) {
        var $mainMenuItems = $("nav[role='navigation'] > ul.menu li.parent-dropdown");
        ...
        for (var i = 0; i < $mainMenuItems.length; i++) {
          ...
          if (i !== $(this).index()) {
            ...
            if ($iItem.hasClass("expanded")) {
              $iItem.removeClass("expanded");
            }
          } else {
            if ($iItem.hasClass("expanded")) {
              $iItem.find("> ul").attr("aria-hidden", "false");
              $iItem.find("> a").attr("aria-expanded", "true");
            } else {
              $iItem.find("> ul").attr("aria-hidden", "true");
              $iItem.find("> a").attr("aria-expanded", "false");
            }
