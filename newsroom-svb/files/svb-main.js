...
//autocomplete
//get site language
    var site_lang = $("html").prop("lang");
    var a = $('#questionform_q');
    var b = $('#mainsearchform_q');

    //if ((a.length > 0) || (b.length > 0)) {
    //    a = $('#questionform_q').autocomplete({
    //        serviceUrl: "/int/" + site_lang + '/system/jsp/json.jsp',
    //        minChars: 2,
    //        zIndex: 9999,
    //        deferRequestBy: 0, //miliseconds
    //        params: { maxAantalResults: '7', taal: site_lang}, //aditional parameters
    //        noCache: false, //default is false, set to true to disable caching
    //        autoSubmit:true,
    //        onSearchStart:  function () {
    //            if(this.value === "Vul 1 of 2 woorden in"){
    //                return false;
    //            }
    //        }
    //    });
    //
    //    b = $('#mainsearchform_q').autocomplete({
    //        serviceUrl: "/int/" + site_lang + '/system/jsp/json.jsp',
    //        minChars: 2,
    //        zIndex: 9999,
    //        deferRequestBy: 0, //miliseconds
    ...
    $("#menu_fontsize").each(
        function(){
            var labelText = "Kies lettergrootte";
            var medium = "Normale letter";
            var large = "Grote letter";
            var extra = "Extra grote letter";
...
$(document).ready(function(){
    ...
    $("ul.showPrint li:first").before("<li><a href='javascript:print()' title='Print' class='print'>Print</a></li>");
    ...
    $(".formblock").each(function(){
        ...
        print_container.append("<a href='javascript:print()' title='Print' class='print'>Print</a>")
    });
});
...
$(document).ready(function(){
    var sDefaultText = "Vul 1 of 2 woorden in";
    ...
    function setDefaultText(theForm){
        ...
        if($("#" + theForm + "_q").prop("value") == ""){
            $("#" + theForm + "_q").prop("value",sDefaultText);
        }
    }
    ...
    function updateOtherForm(currentForm){
        var currectValue = $("#" + currentForm + "_q").prop("value");;
        switch(currentForm){
            case "questionform":
        ...
        $("#" + otherForm + "_q").prop("value",currectValue);
    }
});
...
$(document).ready(function(){
    ...
    $(".vestigingenkaart").each(function(){
        ...
        $("a",this).bind("mouseover",function(){
            var showArea = $(this).prop("class").split(' ').slice(0,1);
            hoverArea(showArea);
        });
        ...
        $(".vestigingenkaart").each(function(){
            ...
            $("img",this).filter(".area").each(function(){
                ...
                if($(this).hasClass(showArea)){
                    $(this).removeClass("toonniet");
                }else{
                    $(this).addClass("toonniet");
                }
            });
        });
    }
...
function wireUpAnchorClick(domAnchor){
    ...
    var isExternal = (anchor.prop("rel") && anchor.prop("rel") == "external");
    var winopt = "";
    if (anchor.hasClass("popup")) {
        winopt = "toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=1257,height=1257,left=0,top=0";
    }
    ...
    if (anchor.hasClass("sitestatcomplink") || anchor.hasClass("sitestatform") || anchor.hasClass("nieuwsoverzicht")) {
        ...
        anchor.click(function(){return ns_onclick(domAnchor, anchor.prop('href'), dataIsland.sitestatid, sitestatType, dataIsland.taal, dataIsland.subsite,winopt);
            window.open(anchor.prop('href'), getNewWindowName() , winopt); return false;
        });
    }
    ...
    if ((anchor.hasClass("sitestat") && isExternal) || anchor.hasClass("sitestatpdf") ) {
        ...
        anchor.click(function(){
            ...
            return ns_onclick(domAnchor, anchor.prop('href'), dataIsland.sitestatid, sitestatType, dataIsland.taal, dataIsland.subsite, winopt);
        });

    } else { if (anchor.hasClass("download") || isExternal || winopt.length > 0) { anchor.click( function(){ window.open(anchor.prop('href'), getNewWindowName() , winopt); return false; } ); } }

    if (anchor.hasClass("sitestatdigid") || anchor.hasClass("sitestaturl")  ) {
        var json = domAnchor.getElementsByTagName("script")[0].innerHTML;
        var dataIsland = eval("(" + json  + ")");
        var sitestatType = "";
        winopt = "toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=1257,height=1257,left=0,top=0";
...
/*
 $(document).ready(function () {
 var $accordionHeader = $('.accordionHeader'),//always cache your selectors
 $accordionContent = $('.accordionContent');

 $accordionContent.each(function () {
 var $accordionHeaderTemp = $(this).prev().clone();
 $accordionHeaderTemp.removeClass("accordionHeader").addClass("accordionHeaderTemp");
 $(this).prepend($accordionHeaderTemp);
 })

 var accordionFunctionOpen = function () {
 var $this = $(this),
 $relatedContent = $this.next();

 // calculate height of title
 // $accordionContent.css({ marginTop: -($this.height() + 13) + 'px', paddingTop: ($this.height() + 9) + 'px' });

 // Close any other opened tabs
 $accordionHeader.show();
 $accordionContent.slideUp('slow');
 $accordionHeader.removeClass('headerActive');

 // Open related tab
 $relatedContent.slideDown('slow');
 $this.addClass('headerActive');

 $(this).hide();
 $relatedContent.find(".accordionHeaderTemp").focus();

 return false;
 };

 var accordionFunctionClose = function () {
 var $this = $(this),
 $relatedContent = $this.parent();

 $relatedContent.slideUp('slow');
 $relatedContent.prev().show();
 $relatedContent.prev().removeClass('headerActive');

 return false;
 };
 $accordionHeader.focus(accordionFunctionOpen);
 $accordionHeader.click(accordionFunctionOpen);
 $(".accordionHeaderTemp").click(accordionFunctionClose);
 });
 */
//#### End accordion####/
//#### BEGIN CODE FOR IFRAME HEIGHT ####/
$(document).ready(function(){
    $(window).on('message',function(e) {
        //if ( e.originalEvent.origin=="http://embed.svbabc.nl" ) $('iframe.embed').height( e.originalEvent.data + 15 );
        $("iframe[src^='" + e.originalEvent.origin + "']").height( e.originalEvent.data + 15 );
    });
});
//#### END CODE FOR IFRAME HEIGHT ####/
//#### BEGIN CODE FOR MAIN MENU TABBING ####/
$(document).ready(function(){
    $('*').on("focus", function(){
        if ($(this).parents('#topmenu li.parent').length == 0) {
            $("#topmenu li ul").css('visibility', '').css('overflow', '');
        }
    });
    $('#topmenu a').off("focus").on("focus", function(){
        // The first li with class parent and then the ul inside should be made visible & overflow...
        $(this).parents('li.parent').first().find('ul').css('visibility', 'visible').css('overflow', 'visible');
    });
});
//#### END CODE FOR MAIN MENU TABBING ####/
...
function ns_onclick (a, url, name, type, svbtaal, svbsubsite, winopt){
    var ns_l="https://nl.sitestat.com/svb/svb/s?";
    ns_l+=name;
...
// publisher subscriber pattern. This pattern is used by responsive.js
function pattern_publisher_subscriber() {
...
function ResponsiveHandler() {
    ...
    var publisher = new pattern_publisher_subscriber();
    ...
    function init() {
        ...
        var responsive_handler = $('<div id="resposive-handler"></div>');
...
/*
* 	Module which handles all interactions on login info dropdown
* */
function LoginInfoDropDown() {

    var currentIndex, minIndex, maxIndex, context;

    function init() {

        currentIndex = -1;
        minIndex = 0;
        context = $('.top-header .drop-down li');
        maxIndex = context.length - 1;
        closeElement = $('.digid-login #close');


        $('.login-info').on('keydown', function (e) {
            if (e.which == 13) {
                toggleLoginOptions();
                e.preventDefault();
            }
        ...
        $(document).on('click', '.login-info',  function (e) {

            toggleLoginOptions()
            e.preventDefault();
        });

        $(document).on('click', '.digid-login #close' , function (e) {
        ...
        $(document).on('keydown', function (e) {
            if ($('.digid-login').hasClass('open')) {
                if ((e.keyCode == 38) || (e.shiftKey && e.keyCode == 9)) { // up
                    navigatePrev(context);
                    e.preventDefault();
                }
    ...
    function showLoginOptions() {
        $('.digid-login').addClass('open');
        $('.overlay').addClass('open');
    }

    function hideLoginOptions() {
        $('.digid-login').removeClass('open');
    ...
    function navigateTo(targetIndex) {
        ...
        var targetElement = context.eq(targetIndex).find('a');
        targetElement.focus();
...
/*
* 	Function to set height of collection to tallest collection item
* */
function equalize(collection) {
    clearEqualization(collection);
    var heighest = 0;
    // collect height
    collection.each(function () {
        var item = $(this);
        if (item.outerHeight() > heighest) {
            heighest = item.outerHeight();
        }
    });

    // equalize collection items
    collection.each(function () {
        var item = $(this);
        item.css('height', heighest + 'px');
    });
}

/*
* 	Function to clear equalized collections. Used when user resizes window from large (equalized collections) to small.
* */
function clearEqualization(collection) {
    collection.css('height', 'auto');
}

/*
* 	Function that equalizes defined collections. Called on window resize event.
* */
function equalizeCollections() {
    equalize($('.news-teaser'));
    equalize($('.campaign-teaser'));
    equalize($('.linkmodule.digid'));
    equalize($('.linkmodule.digid .drop-down'));
    equalize($('.regulation-highlight'));
    equalize($('.all-regulations a'));
    $('.pt-landing .navigationblock .list').each(function () {
        equalize($(this).children('div'));
    });
}

/*
 * 	Function that clears equalizes on defined collections. Called on window resize event.
 * */
function clearEqualizedCollections() {
    clearEqualization($('.news-teaser'));
    clearEqualization($('.campaign-teaser'));
    clearEqualization($('.linkmodule.digid'));
    clearEqualization($('.linkmodule.digid .drop-down'));
    clearEqualization($('.regulation-highlight'));
    clearEqualization($('.all-regulations a'));
    clearEqualization($('.pt-landing .navigationblock .list > div'));
}


// init responsive module
if (typeof ResponsiveHandler == 'function') {
    var responsiveHandler = new ResponsiveHandler();
    responsiveHandler.init();

    setTimeout(function () {
        responsiveHandler.republish();
    }, 50);
}

// init login info drop down interactions
if (typeof LoginInfoDropDown === 'function') {
    var loginInfoDropDown = new LoginInfoDropDown();
    loginInfoDropDown.init();
}

if ($('.field-date').length && typeof FieldDate == "function") {
    var fieldDate = new FieldDate();
    fieldDate.init();
}

//	Language list logics
var languageList = $('#languagelist');

languageList.val($('html').attr('lang'));


if (languageList.val() !== 'nl') {

    languageList.removeClass('nl');
}
...
// disable selectbox if only one language
if (languageList.find('option').length <= 1) {
    languageList.prop('disabled', 'disabled');
    languageList.closest('.language-select').addClass('disabled');
}

//  prevent auto suggestion close when click on questionform
$('.autocomplete-suggestions, .questionform_q').on('click', function (e) {
    e.stopPropagation();
});

// escape press or pagetype click should deactivate popups and popovers
$('[class*=pt-]').on('click', function () {
...
// help function for toggle status cookie
function getURLPath() {
    return escape(window.location.pathname);
}

// toggle linkmodule content
$('.linkmodule h2').on('click', function () {
    $(this).parent().toggleClass('open');
});

// toggle regulations
$('.all-regulations').each(function () {
    var SavedState = readCookie("SavedState_" + getURLPath() + "_" + $("*").index($(this)));
    if (SavedState == null) {
        $(this).hide();
    } else {
        $(this).closest('.all-regulations-block').toggleClass('open');
    }
})

$('.show-all-regulations').on('click', function (e) {
    e.preventDefault();
    var $allRegulationsBlock = $(this).closest('.all-regulations-block');
    var $allRegulations = $allRegulationsBlock.find('.all-regulations');
    if ($allRegulationsBlock.hasClass('open')) {
        setCookie("SavedState_" + getURLPath() + "_" + $("*").index($($allRegulations)), "", "/", -1)
    } else {
        setCookie("SavedState_" + getURLPath() + "_" + $("*").index($($allRegulations)), true, "/", 1);
    }
    $allRegulationsBlock.toggleClass('open');
    $allRegulations.slideToggle();
});

/*
* 	Equalize / Clear equalizations on resize event.
* 	There is a timeout on resize for resize end simulation.
* */
var resizeTimeout;
$(window).on('resize', function (e) {
    ...
    resizeTimeout = setTimeout(function () {
        ...
        if (responsiveHandler.breakpointActive('tablet-portrait')) {
            equalizeCollections();
        } else if (responsiveHandler.breakpointActive('tablet-landscape')) {
            equalizeCollections();
        } else {
            clearEqualizedCollections();
        }
...
if (responsiveHandler.breakpointActive('tablet-landscape')) {
    equalizeCollections();
}

// Video accoridon
$('.download_video_link a').on('click', function (e) {
    e.preventDefault();

    if ($('.download_video_link a').hasClass('open')) {
        $('.download_video_link a').removeClass('open')
        $('.downloads ul').removeClass('open')
        $('.download_video_link a').addClass('close');
        $('.downloads ul').addClass('close');

    } else {
        $('.download_video_link a').removeClass('close')
        $('.downloads ul').removeClass('close')
        $('.download_video_link a').addClass('open');
        $('.downloads ul').addClass('open');
    }
})

$('.video_transcript_link a').on('click', function (e) {
    e.preventDefault();

    if ($('.video_transcript_link a').hasClass('open')) {
        $('.video_transcript_link a').removeClass('open')
        $('.transcription > div').removeClass('open')
        $('.video_transcript_link a').addClass('close');
        $('.transcription > div').addClass('close');

    } else {
        $('.video_transcript_link a').removeClass('close')
        $('.transcription > div').removeClass('close')
        $('.video_transcript_link a').addClass('open');
        $('.transcription > div').addClass('open');
    }
})

//uitklapper accordion
var allPanels = $('.faqList .accordionContent').hide();

$('.faqList .accordionHeader').on('click', function (e) {
    e.preventDefault();
...
//// JQUERY FORM TOOLTIP
//$(document).ready(function () {
//    $(".formblock a.hint").tooltip({
//        tooltipClass: "form-tooltip",
//    });
//    $(".formblock a.alert").tooltip();
//
//});

// Sitemap page toggle navigation

$('.navigationblock').each(function () {
    var SavedState = readCookie("SavedState_" + getURLPath() + "_" + $("*").index($(this)));
    if (SavedState == null) {
        //$(this).hide();
        $(this).find('.moreorless').hide();
        $(this).find('.toggle.label .up').hide();
    } else {
        $(this).find('.toggle.label .down').hide();
    }
})

$('.navigationblock .toggle.label .down').on('click', function (e) {
    e.preventDefault();
    var $navBlock = $(this).closest('.navigationblock');
    $navBlock.find('.moreorless').slideDown();
    $navBlock.find('.up').show();
    $navBlock.find('.down').hide();
    $navBlock.addClass('open');
    setCookie("SavedState_" + getURLPath() + "_" + $("*").index($navBlock), true, "/", 1);
});

$('.navigationblock .toggle.label .up').on('click', function (e) {
    e.preventDefault();
    var $navBlock = $(this).closest('.navigationblock');
    $navBlock.find('.moreorless').slideUp();
    $navBlock.find('.up').hide();
    $navBlock.find('.down').show();
    $navBlock.removeClass('open');
    setCookie("SavedState_" + getURLPath() + "_" + $("*").index($navBlock), "", "/", -1)
});


//CHANGE SEARCH Placeholder
var $questionform = $('#questionform_q');
var largeSearchTxt = $questionform.attr('data-ph-large');
var smallSearchTxt = $questionform.attr('data-ph-small');

function responsiveSearch() {
    if ($questionform.prop("value") == "") {
        $questionform.css("color", "silver");
    }

    if ($(window).width() < 721) {
        $questionform.attr('placeholder', smallSearchTxt);
    }
    else {
        $questionform.attr('placeholder', largeSearchTxt);
    }
};
responsiveSearch();

//attach onfocus event to input field
$questionform.bind("focus", function () {
    $questionform.removeAttr('placeholder');
    $questionform.css("color", "#1C1C1C");
});

//attach onblur event to input field
$questionform.bind("blur", function () {
    responsiveSearch();
});

// Mobile Slide Linke Module DIGID Begin
if ($(window).width() < 721) {
    $('.linkmodule.digid .drop-down').hide();
    $('.linkmodule.digid').removeClass('open');
    $('.linkmodule.digid').addClass('closed');
}

function responsiveLogin() {
    if ($(window).width() < 721) {
        $('.linkmodule.digid  .title').prop('tabIndex', 0);

        function toggle(toggler) {

            if (toggler.parent().hasClass('open')) {
                toggler.parent().removeClass('open');
                toggler.parent().addClass('closed');
                toggler.parent().children('.drop-down').slideUp();

            } else {
                toggler.parent().removeClass('closed');
                toggler.parent().addClass('open');
                toggler.parent().children('.drop-down').slideDown();
            }
        }

        $('.linkmodule.digid .title').off('click').on('click', function (e) {
            e.preventDefault();
            toggle($(this));
        });

        $('.linkmodule.digid .title').off('keypress').on('keypress', function (e) {
            if (e.which == 13) {
                toggle($(this));
            }
        });
        ...
        $('.linkmodule.digid .drop-down').show();
        $('.linkmodule.digid').removeClass('closed');
        $('.linkmodule.digid').addClass('open');

        $('.linkmodule.digid  .title').off('click').on('click', function (e) {
            e.preventDefault();
        });
        $('.linkmodule.digid  .title').prop('tabIndex', -1);
    }
}

responsiveLogin();

// Mobile Slide Linke Module DIGID END
//SearchTips Toggle
$('.searchtips').hide();

$('.searchtips-link').on('click', function (e) {
    e.preventDefault();

    $('.searchtips-block').toggleClass('open');
    $('.searchtips').slideToggle();
})

// Wizard Focus
$('#wizardfocus').on('focus', function () {
    $(this).parent().find('input:enabled')[0].focus();
})

// Remove not used Skiplinks
$('#skiplinks a').each(function () {
    var anchorId = $(this).attr('href');
    if ($(anchorId).length == 0 || $(anchorId).prop("disabled")) {
        $(this).remove();
    }
})
